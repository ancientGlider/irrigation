...
#define PIN_SOIL_TRH        Q1


#define NPKADDR             0x02        // адрес сенсора NPK, pH, EC, RH, temp
#define TRHADDR             0x03        // адрес сенсора temp, RH почвы
#define SENSORMAXDATA       11          // максимальное количество байт в ответе датчика
#define STAB_PAUSE          1500        // время стабилизации датчика после включения питания
...
#define STAB_PAUSE          5000        // время стабилизации датчика после включения питания
...
byte readSoilSensors() {
  ...
  for (byte i = 0; i < 3; i++) {    // снимаем показания 3 раза через 2 сек для повышения точности
    delay(STAB_PAUSE);          // пауза 15 сек для стабилизации показаний
...
#ifdef DEBUG
  Serial.println("readPin : " + String(pin));
#endif   
  if (pin & 0x100) return shiftRead(pin & 0xFF);
  else return digitalRead(pin);
}

void writePin(int pin, uint8_t data) {
  if (pin & 0x100) shiftWrite(pin & 0xFF, data);
  else digitalWrite(pin, data);
}

uint8_t shiftRead(uint8_t pin) {
#ifdef DEBUG
  Serial.println("shiftRead : " + String(pin));
#endif
  readShiftRegs();
  return shiftInput & (1 << pin) ? HIGH : LOW;
}

void shiftWrite(uint8_t pin, uint8_t data) {
  uint8_t oldState = shiftOutput;
  shiftOutput = data ? bitSet(shiftOutput, pin & 0x0F) : bitClear(shiftOutput, pin & 0x0F);
  if (shiftOutput != oldState) writeShiftRegs();
}

void writeShiftRegs() {
#ifdef DEBUG
    Serial.println("writeShiftRegs : " + String(shiftOutput));
#endif
    // записываем значения в буфер
    FOR(8) {
        digitalWrite(PIN_SHIFT_TX, shiftOutput & 1 << (7 - i) ? HIGH : LOW);
        digitalWrite(PIN_SHIFT_SYNC_TX, HIGH);
//        delayMicroseconds(PULSE_WIDTH_USEC);
        digitalWrite(PIN_SHIFT_SYNC_TX, LOW);
    }
    
    // передаем записанные в регистр значения на пины
    digitalWrite(PIN_SHIFT_LATCH_TX, LOW);
//    delayMicroseconds(PULSE_WIDTH_USEC);
    digitalWrite(PIN_SHIFT_LATCH_TX, HIGH);
}
...
void readShiftRegs() {
    ...
    digitalWrite(PIN_SHIFT_LATCH_RX, LOW);
//    delayMicroseconds(PULSE_WIDTH_USEC);
    digitalWrite(PIN_SHIFT_LATCH_RX, HIGH);

    // считываем полученные данные о пинах
    FOR(8){
        uint8_t bitVal = digitalRead(PIN_SHIFT_RX);
        shiftInput |= (bitVal << (7 - i));
        digitalWrite(PIN_SHIFT_SYNC_RX, HIGH);
//        delayMicroseconds(PULSE_WIDTH_USEC);
        digitalWrite(PIN_SHIFT_SYNC_RX, LOW);
    }
